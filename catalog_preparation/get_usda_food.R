...
OUTPUT_FILE_NAME = 'data/food/food_usda.tsv'
...
data.sub.plain$group <- str_replace_all(data.sub.plain$group, " ", "_")

# now pluralize
plural <- pluralize(data.sub.plain$word)
plural.group <- data.table(cbind(data.sub.plain$group, plural))
setnames(plural.group, c("group", "word"))
data.sub.plain <- rbind(data.sub.plain, plural.group)

# now capitalize
capitalized <- capitalize(data.sub.plain$word)
capitalized.group <- data.table(cbind(data.sub.plain$group, capitalized))
setnames(capitalized.group, c("group", "word"))
data.sub.plain <- rbind(data.sub.plain, capitalized.group)
