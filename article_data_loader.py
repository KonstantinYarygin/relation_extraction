...
class ArticleCatalog(object):
    ...
    def __init__(self, path):
        ...
        if self.type == 'full':
            ...
            medline_citation_records = (record for record in medline_citation_records if record is not None)
    ...
    def xml_abstract_parse(self, medline_citation_record):
        ...
        abstract_record_chunks = abstract_record.findall('AbstractText') if abstract_record is not None else []
        abstract_chunks = [sub_record.text for sub_record in abstract_record_chunks if sub_record.text is not None]
