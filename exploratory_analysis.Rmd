...
data <- fread('~/Downloads/sentences_first_with_journals.csv', sep=',')
```
Структура данных:
```{r}
nrow(data)
...
```{r}
data.sub <- data[1950:2000]
data.bacteria = data[,
         .(unlist(llply(str_match_all(bacteria, '\'([^\\)]+)\''), .fun=function(x) x[,2])),
           unlist(llply(str_match_all(bacteria, ',\\s(\\d+)\\)'), .fun=function(x) x[,2]))),
         by=.(text, article_title)]
setnames(data.bacteria, c("text","article_title","bacteria","code"))
...
data.disease = data[,.(unlist(llply(str_match_all(diseases, '\'([^\\)]+)\','), .fun=function(x) x[,2])),
                      unlist(llply(str_match_all(diseases, ',\\s\'([^\\)]+)\''), .fun=function(x) x[,2]))),
         by=.(text, article_title)]

setnames(data.disease, c("text","article_title","disease","code"))
```
Присоединим импакты
```{r}
