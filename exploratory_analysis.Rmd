...
# merged <- merge(data.bacteria, data[,.(text, article_title)], by=c('text', 'article_title'), all=TRUE)
# merged <- merge(merged, data.nutrient, by=c('text', 'article_title'), all=TRUE)
# merged <- merge(merged, data.disease, by=c('text', 'article_title'), all=TRUE)
# todo: продолжить мержить
```
