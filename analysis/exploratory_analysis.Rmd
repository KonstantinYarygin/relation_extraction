...
data.food <- data[,.(tolower(unlist(llply(str_match_all(food, 
                                                  '\'([^\\)]+)\','), 
                                    .fun=function(x) x[,2]))),
...
data.sentences.diseases.short <- data.sentences.diseases[,.(paste(unique(bacteria), collapse = ";"),
                                                            paste(unique(disease), collapse = ";")),
                                                         by=c("text", "article_title", "journal")]
setnames(data.sentences.diseases.short, c("text", "article_title", "journal", "bacteria", "disease"))
write.table(x = unique(data.sentences.diseases.short[,.(text, bacteria, disease)]), 
            file = "disease-texts.csv", sep = '\t', quote = F, row.names = F, col.names = F)
